  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const { data: followingData } = await api.get<any[]>(`/users/${org}/following`)
      ...
      <main className="flex-1 max-w-[1280px] w-full mx-auto px-4 md:px-8 py-8 flex flex-col md:flex-row gap-8">
        ...
        <div className="w-full md:w-[296px] shrink-0 -mt-10 md:mt-0 relative z-10">
          ...
          <div className="flex items-center gap-1 text-sm text-[var(--text-secondary)] mb-4">
            ...
            <Link href={`/${profile.username}/followers`} className="text-[var(--text-secondary)] hover:text-blue-500 flex items-center gap-1">
              <span className="font-semibold">{profile.follower_count ?? 0}</span> followers
            </Link>
            <span className="mx-1">·</span>
            <Link href={`/${profile.username}/following`} className="text-[var(--text-primary)] hover:text-blue-500 flex items-center gap-1 font-semibold">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-6 pb-2 border-b border-[var(--glass-border)]">
             <div className="flex gap-4">
                 <Link href={`/${profile.username}/followers`} className="text-[var(--text-secondary)] hover:text-[var(--text-primary)] pb-2 px-2 flex items-center gap-1.5">
                 ...
                 <Link href={`/${profile.username}/following`} className="font-semibold text-blue-500 border-b-2 border-blue-500 pb-2 px-2 flex items-center gap-1.5">
          ...
          <div className="space-y-4">
            {following.length === 0 ? (
               ...
               <div key={user.id} className="glass-card p-5 rounded-2xl border border-[var(--glass-border)] flex items-start gap-4 transition-all hover:scale-[1.01]">
                  <Link href={`/${user.username}`}>
                    {user.avatar_url ? (
                      <img src={user.avatar_url} alt={user.username} className="w-12 h-12 rounded-full object-cover border border-[var(--glass-border)]" />
                    ) : (
                  ...
                  <div className="flex-1">
                     <div className="flex items-center gap-2 mb-1">
                        <Link href={`/${user.username}`} className="font-semibold text-[var(--text-primary)] hover:text-blue-500 hover:underline">
                           {user.full_name || user.username}
                        </Link>
                        <Link href={`/${user.username}`} className="text-sm text-slate-400 hover:text-blue-500">
                           @{user.username}
                        </Link>
                     </div>
                     {user.bio && (
