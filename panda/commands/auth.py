...
@click.group()
def token():
    """Manage Personal Access Tokens (used for `git push`/`clone` over HTTPS)."""
...
@click.option(
    "--scopes",
    default="repo",
    help="Comma-separated scopes (default: repo)",
)
@click.option(
    "--expires-in-days",
    type=int,
...
def token_revoke(token_id: str):
    """Revoke a Personal Access Token by its ID (see `panda token list`)."""
    try:
