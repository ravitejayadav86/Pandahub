...
Stores the logged-in user's tokens and the API base URL in
``~/.pandahub/config.json``. This file contains secrets (access/refresh
tokens) so it's written with owner-only permissions where the OS supports it.
"""
...
DEFAULT_API_BASE_URL = "https://pandahub-backend.onrender.com/api/v1"
...
def load_config() -> dict[str, Any]:
    """Load the config file, or return defaults if it doesn't exist yet."""
